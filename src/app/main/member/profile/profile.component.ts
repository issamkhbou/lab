...
export class ProfileComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.memberService.getMemberByEmail(this.memberEmail).then(member => {
      ...
      this.memberService.getPostsByAuthorId(this.id.toString()).then(
        pubs => {
          this.publications = pubs;
        }
      );

      this.memberService.geteventByAuthorId(this.id.toString()).then(
        events => {
          this.evenements = events;
        }
      );

      this.memberService.getToolsByAuthorId(this.id.toString()).then(
        tools => {
          this.outils = tools;
        }
      );
