...
export class MemberService {
  ...
  getMemberByEmail(email: string): Promise<Member> {
    ...
    return new Promise((resolve) =>
      resolve(
        this.placeholderMembers.filter((item) => item.email === email)[0] ??
          null
      )
    );
  }
  ...
  saveMember(member: any): Promise<Member> {
    ...
    const memberToSave = {
      ...
      dateNaissance: member.dateNaissance ?? new Date().toISOString(),
      ...member,
    };
    this.placeholderMembers = [
      memberToSave,
      ...this.placeholderMembers.filter((item) => item.id !== member.id),
    ];
  ...
  removeMemberById(id: string): Promise<void> {
    ...
    this.placeholderMembers = this.placeholderMembers.filter(
      (item) => item.id !== id
    ); // all but the to be deleted id
